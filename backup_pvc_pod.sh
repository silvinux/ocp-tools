		...
		count=${#mountpoints[@]}
        	for i in $(seq 1 $count);do
	    		...
	    		if [[ -d ${pathbackup}/${namespace}/$(date +%Y%m%d)/${POD} ]];then
            		...
            		echo "------------------------------------"
	    		echo "COPYING DATA FROM ${POD}:${mountpoints[$i-1]} TO ${pathbackup}/${namespace}/$(date +%Y%m%d)/${POD}/ -n ${namespace}"
            		echo "------------------------------------"
            		echo "oc rsync ${POD}:${mountpoints[$i-1]} ${pathbackup}/${namespace}/$(date +%Y%m%d)/${POD}/ -n ${namespace}"
            		oc rsync ${POD}:${mountpoints[$i-1]} ${pathbackup}/${namespace}/$(date +%Y%m%d)/${POD}/ -n ${namespace}
		    fi
		done
